class Nearby {
  ...
  String[] findAllNearbyStr(int myId, int xBuffer, int yBuffer) {
    String[] ret = new String[vars.objectLocs.length];
    int index = 0;
    for(int i = 1; i < vars.objectLocs.length ; i++) {
      if (
        //Left Sid + Dist to Right+buffer :is greater than: left of target object
        ...
        index++;
        ret[index] = vars.objectLocs[i].objectName;
      }
    }
    return ret;
  }

  int[] findAllNearbyInt(int myId, int xBuffer, int yBuffer) {
    int ret[] = new int[vars.objectLocs.length];
    int index = 0;
    for(int i = 1; i < vars.objectLocs.length ; i++) {
      if (
        //Left Sid + Dist to Right+buffer :is greater than: left of target object
